...
qacServices.getAdminPage = function getAdminPage(req){
    ...
    return qacServices.getOrganizations().then(function(orgs) {
        if(orgs.length) {
            ...
            var trs = [];
            trs.push(html.tr([html.td('There are no organizations')]));
            if(qacServices.validSession(req)) {
                trs.push(html.table([newOrgTR]));
                return html.form({method:'post', action:qacServices.rootUrl},trs);
            } else {
                return html.table(trs);
            }
        }
    });
};
