    ...
    return qacServices.getOrganizations().then(function(orgs) {
        if(orgs.length) {
            return Promises.all(orgs.map(function(org) {
                ...
                for(var tr in trs) { all_trs.push(trs[tr]); }
                if(qacServices.validSession(req)) {
                    all_trs.push(html.tr([html.td({colspan:2, "class":'right-align'}, qacServices.orgAddButton(req))]));
                }
                if(qacServices.validSession(req)) {
                    return html.form({method:'post', action:qacServices.rootUrl}, [html.table(all_trs)]);
                } else {
...
function existsOnGitHubInCategory(category, organization, project){
    return Promises.start(function() {
        return request(qacServices.gitHubUrl(category, organization));
    }).then(function(repos) {
...
qacServices.doRepoUpdate = function doRepoUpdate(info, repo_url, feedback, bitacora) {
    ...
    return fs.stat(clonePath).then(function() {
        ...
        var count={
            error  :{count:0, abr:'err', color:'red'   },
            obs    :{count:0, abr:'obs', color:'yellow'},
            warning:{count:0, abr:'war', color:'orange'},
            notice :{count:0, abr:'not', color:'ffdb99'}, // naranja claro
        }
        var scoreTypes = {
        ...
        qaControlWarnings.forEach(function(warn){
            //console.log(warn);
            if(warn.warning==='older_version_of_qa_control_in_package_json') {
                qaControlLastVersion = false;
                return; // ignore this specific warning!
            }
            warn.scoring = warn.scoring || {fatal:1};
            for(var scoreName in warn.scoring){
                count[(scoreTypes[scoreName]||scoreTypes.fatal).gravity].count+=warn.scoring[scoreName];
            }
        })
        // var label = 'ok', color='green';
        var label;
        var color;
        for(var gravity in count){
            var county = count[gravity];
            if(!label && county.count){
                label=county.count+' '+county.abr;
                color=county.color;
            }
        }
        if(!label){
            label='ok';
            //console.log(qaControlLastVersion ? 'LAST version' : 'OLDER version');
            if(qaControlLastVersion) {
                color = 'brightgreen';
            }else{
                color =  'green';
            }
