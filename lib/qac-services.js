...
qacServices.cucardasToHtmlList = function cucardasToHtmlList(cucardas) {
    ...
    var result = [];
    cucardas.replace(new RegExp(e_cucarda,'g'),function(cucarda, name, imgSrc, name2, imgSrc2, url){
        var htmlCucarda;
        htmlCucarda = html.img({src:imgSrc||imgSrc2, alt:name||name2});
        if(url){
            htmlCucarda = html.a( {href:url}, [htmlCucarda]);
        }
        result.push(htmlCucarda);
    });
    return result;
};
        ...
        if(info.organization.projects.length) {
            return Promises.all(info.organization.projects.map(function(project) {
                return fs.readFile(Path.normalize(info.organization.path+'/projects/'+project.projectName+'/result/cucardas.md'), 'utf8').catch(function(err) {
                    ...
                    if(qacServices.user) {
                        //tds.push(html.td(makeButton('Delete', qacServices.rootUrl+'delete/'+organization+'/'+project.projectName, 'Delete project \\\''+project.projectName+'\\\'')));
