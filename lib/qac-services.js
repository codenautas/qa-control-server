...
qacServices.globalPushStatusPath = function globalPushStatusPath() {
    return Path.normalize(qacServices.repository.path+'/groups/push-errors.jsons');
};

qacServices.projectPushStatusPath = function projectPushStatusPath(info) {
    return Path.normalize(info.project.path+'/result/push-status.json');
};

function writePushStatus(info, data) {
    return writeJsonToFile(qacServices.projectPushStatusPath(info), data);
};

function appendPushError(data) {
    return fs.appendFile(qacServices.globalPushStatusPath(), JSON.stringify(data, null, 4)+'\n', {encoding:'utf8'});
};
...
qacServices.logFatalError = function logFatalError(message, req) {
    ...
    function saveLog(repo, message, data) {
        return Promises.start(function() {
            if(repo && repo.organization) {
                return qacServices.getInfo(repo.organization, repo.name);
            }
            return false;
        }).then(function(info) {
            if(info) {
                return writePushStatus(info, data);
            } else {
                return appendPushError(data);
            }
        }).catch(function(err){
            data.message += ': '+err.message;
            //data.req = { headers: req.headers, body:req.body || 'null body' };
            return appendPushError(data);
        });
    };
    saveLog(repo, message, logData);
};
...
qacServices.doRepoUpdate = function doRepoUpdate(info, repo_url, feedback, bitacora) {
    ...
    return writePushStatus(info, { date:dateForFileName(), status:'Updating repo...', info:info } ).then(function() {
        ...
        return writePushStatus(info, { date:dateForFileName(), status:'Cloning...', info:info } );
    }).then(function(){
        ...
        return writePushStatus(info, { date:dateForFileName(), status:'qa-control...', info:info } );
    }).then(function() {
        ...
        return writePushStatus(info, { date:dateForFileName(), status:'qa-control evaluation...', info:info } );
    }).then(function(){
        ...
        return writePushStatus(info, { date:dateForFileName(), status:'error finalizando!', info:info, err:err } );
    });
};
