...
qacServices.setSession = function setSession(req) {
    //qacServices.user = (req.session||{passport:{}}).passport.user;
...
qacServices.validSession = function validSession() {
    // console.log("CHECK USER", qacServices.user);
    return !!qacServices.user;
};
...
qacServices.getProject = function getProject(info, project) {
    ...
    return qacServices.getInfo(organization).then(function(info) {
        if(info.organization.projects.length) {
            return Promises.all(info.organization.projects.map(function(project) {
                return qacServices.getProject(info, project);
            })).then(function(trs) {
                //console.log("trs", trs);
...
qacServices.getProjectPage = function getProjectPage(organization, project){
    ...
    return qacServices.getInfo(organization, project).then(function(nfo) {
        info = nfo;
        return qacServices.getProject(info, {projectName:project});
...
function handleAbms(thisModule, url, method, pref) {
    ...
    this.handle = function(req, res, next) {
        ...
        thisModule.user = thisModule.setSession(req);
        var doAction=null;
        switch(vars.action) {
