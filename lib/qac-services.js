...
qacServices.getProjectLogs = function getProjectLogs(projectPath) {
    ...
    return fs.readJSON(Path.normalize(projectPath+'/result/qa-control-result.json'), 'utf8').catch(function(err) {
        ...
        if(qac.length) {
            ...
            trs.unshift(html.tr([html.td({colspan:3}, 'QA Control result')]));
...
qacServices.getProjectPage = function getProjectPage(organization, project){
    ...
    return qacServices.getInfo(organization, project).then(function(nfo) {
        ...
        //console.log("antes del log", content.toHtmlText({pretty:true, title:'prueba'}));
        pageCont = [content];
        ...
        return html.table([html.tr([html.td(content)])]);
    }).catch(function(err) {
...
qacServices.staticServe = function staticServe(){
    var thisModule = this;
    return app.get(thisModule.rootUrl+':filename',function(req,res,next){
        if(req.params.filename.match(/(.(css|jpg|png|gif|ico|js))$/)) {
            console.log("Serving static", req.params.filename);
...
// qacServices.askServe = function askServe(){
    // var thisModule = this;
    // return app.use(thisModule.rootUrl+'ask',function(req,res){
        // var o=html.form({method:'post', action:thisModule.rootUrl+req.path.substring(req.path[0]==='/'?1:0)},[
            // html.p({style:'color:gray'}, thisModule.rootUrl),
            // html.h2(["Proceed with: "+req.path+" ?"]),
    ...
    return app.get(thisModule.rootUrl+':organization',function(req,res,next){
        if(req.params.organization.match(/(.(css|jpg|png|gif|ico|js))$/)) {
            ...
            thisModule.getOrganizationPage(req.params.organization).then(function(content){
                noCacheHeaders(res);
                console.log("organizationServe content", content);
...
qacServices.projectServe = function projectServe(){
    ...
    return app.get(thisModule.rootUrl+':organization/:project',function(req,res,next){
        console.log("CALLED projectServe-------------------- ", req.params);
        if(req.params.organization.match(/(.(css|jpg|png|gif|ico|js))$/)) {
            return next();
            return res.sendFile(Path.resolve('./app/'+req.params.organization));
        } else if(req.params.organization.match(/^(login|admin|(manual-)?(delete|create|add))$/)) {
            ...
            thisModule.getProjectPage(req.params.organization, req.params.project).then(function(content){
                noCacheHeaders(res);
