        ...
        return fs.stat(info.organization.path).catch(function(err) {
            ...
            if(!!project) {
                ...
                if(! projectFound.length) {
                    throw new Error('inexistent project "'+project+'"');
                }
                info.project = {
...
qacServices.receiveManualPush = function receiveManualPush(){
    ...
    return app.get('/manual-push/:organization/:project',function receiveManualPushService(req,res){
        // validar request
        var repo={
        ...
        actualizeRepo(repo, res, Date());
    });
};

function actualizeRepo(repo, res, timestamp){
    var info;
    ...
    qacServices.getInfo(repo.organization, repo.name).then(function(nfo) {
        ...
        }],{echo:true, collect:true, exit:true}).catch(function(err){
            console.log('-------------',err);
            throw err;
        });
        ...
        console.log("qac-services.actualizarRepo err", err);
    ...
    return qacServices.getInfo(organization, project).then(function(info) {
        ...
        if(!!project) {
            ...
            return fs.readFile(resultsDir+'cucardas.md', 'utf8').catch(function(err) {
                ...
                if(qacServices.user) {
                    r += '|'+makeButton('Delete', '/delete/'+organization+'/'+project, 'post', 'Delete project \\\''+project+'\\\'')
                }
                r = header+r+'\n';
            ...
            if(qacServices.user) {
                var oclick = 'var p=this.form.project; var org=this.form.organization.value;'
                            +' if(p.value != \'\') { this.form.action=\'/add/\'+org+\'/\'+p.value;'
            ...
            if(info.organization.projects.length) {
                return Promises.all(info.organization.projects.map(function(project) {
                    return fs.readFile(Path.normalize(info.organization.path+'/projects/'+project.projectName+'/result/cucardas.md'), 'utf8').catch(function(err) {
                        ...
                        var r = '['+project.projectName+']('+'/'+organization+'/'+project.projectName+') '+'|'+content.split('\n').join(' ');
                        if(qacServices.user) {
                            r += '|' + makeButton('Delete', '/delete/'+organization+'/'+project.projectName, 'post', 'Delete project \\\''+project.projectName+'\\\'')
                        }
                        return r;
                    });
...
function handleAbms(thisModule, pref) {
    this.url = '/:action/:organization/:project?';
    ...
    return app.get('/admin', function(req,res,next){
        ...
        qacServices.getOrganizations().then(function(orgs) {
            ...
            for(var o=0; o<orgs.length; ++o) {
                var org = orgs[o];
                out += '['+org+']('+org+') '
                       + '|' +makeButton('Delete', '/delete/'+org, 'post', 'Delete organization \\\''+org+'\\\'', okJS)+'\n';
            }
            var oclick = 'var o=this.form.organization;'
                        +' if(o.value != \'\') { this.form.action=\'/create/\'+o.value;'
                        +' this.form.method = \'post\'; this.form.submit(); }'
                        +' else { o.focus(); }';
