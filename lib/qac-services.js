    ...
    return qacServices.getInfo(organization).then(function(info) {
        if(info.organization.projects.length) {
            return Promises.all(info.organization.projects.map(function(project) {
                ...
                var tds = [ html.th('project'), html.th({colspan:qacServices.cucardasToHtmlList.order._OTHERS_},'cucardas')];
                if(qacServices.user) {
            ...
            return html.table([html.tr([html.td('No projects')])]);
        }
    });
};
...
function handleAbms(thisModule, pref) {
    ...
    this.handle = function(req, res, next) {
        ...
        doAction(req.params.organization, req.params.project, pref !== '' ? new Feedback(res) : new NoFeedback()).then(function(content) {
            res.end(content); 
        }).catch(function(err) {
            res.end(err.message);
        });
