...
qacServices.overviewServe = function overviewServe(){
    ...
    return app.get('/:group/:project?',function(req,res,next){
        if(req.params.group.match(/(.css)$/)) {
            var css = Path.resolve('./app/'+req.params.group);
            return fs.readFile(css, 'utf8').then(function(content) {
                res.end(content);
            }).catch(function(err) {
                res.end(err.message);
            });
        } else {
            thisModule.makeOverviewHtml(req.params.group, req.params.project).then(function(content){
