...
qacServices.getInfo = function getInfo(organization, opts){
    ...
    return Promises.start(function() {
        ...
        return fs.stat(info.organization.path).catch(function(err) {
            ...
            return fs.readFile(Path.normalize(info.organization.path+'/params/projects.json'),'utf8');
...
qacServices.manageDeletes = function manageDelete(organization, project){
    ...
    return qacServices.getInfo(organization, opts).then(function(info) {
        ...
        if(!!project) {
            var dirToRemove=Path.normalize(info.organization.path+'/projects/'+project);
...
qacServices.overviewServe = function overviewServe(){
    ...
    return app.get('/:organization/:project?',function(req,res,next){
        ...
        } else if(req.params.organization.match(/^(login|admin)$/)) {
            return next();
        } else {
