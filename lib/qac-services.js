...
function Bitacora(outputDir) {
    ...
    this.finish = function() {
        var lg = JSON.stringify(this.data);
        var outFile = this.outputDir+this.now()+'.json';
        fs.writeFile(outFile, lg, {encoding:'utf8'}).then(function() {
            //console.log("bitacora guardada", outFile);
        });
    };
};
...
qacServices.receivePush = function receivePush(){
    return app.post('/push/:group/:project',function receivePushService(req,res){
        ...
        qacServices.getInfo(repo.organization, {project:repo.name, createProject:true}).then(function(nfo) {
            ...
            bitacora.log('command', result);
            if(result.exit != 0) {
                bitacora.faile(new Error("git failed with code"));
            }
