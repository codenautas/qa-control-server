...
function fixOrganizationInReq(req) {
    //WLF("repo.full_name:"+JSON.stringify(req.body.repository.full_name)+" repo.organization:"+JSON.stringify(req.body.repository.organization));
...
qacServices.globalPushStatusPath = function globalPushStatusPath() {
    return Path.normalize(qacServices.repository.path+'/groups/push-errors.json');
};
...
function appendPushError(data) {
    var lpFile = qacServices.globalPushStatusPath();
    return fs.exists(lpFile).then(function(exists) {
        if(exists) {
            return fs.readJson(lpFile);
        }
        return [];
    }).then(function(pData) {
        pData.push(data);
        return fs.writeFile(lpFile, JSON.stringify(pData, null, 4)+'\n', {encoding:'utf8'});
    });
};
