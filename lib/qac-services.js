...
qacServices.projectActionButtons = function projectActionButtons(organization){
    var tds=[];
    return [td.html("x")];
    if(qacServices.user) {
        tds.push(html.td([
    ...
    if(qacServices.user && !qacServices.production){
        tds.push(html.td([
            html.a({
                href:qacServices.rootUrl+'manual-push/'+organization+'/'+project.projectName
    ...
    return qacServices.getInfo(organization).then(function(info) {
        if(info.organization.projects.length) {
            return Promises.all(info.organization.projects.map(function(project) {
                ...
                if(qacServices.user) {
                    return html.form({method:'post'}, [html.table(all_trs)]);
                } else {
...
app.use(qacServices.rootUrl+'/ask',function(req,res){
    var o=html.form({method:'get', action:qacServices.rootUrl+req.path},[
        html.p({style:'color:gray'}, qacServices.rootUrl),
        html.h2(["Confirm to proced with: "+req.path+" ?"]),
        html.input({type:'submit'}, 'Ok')
    ]);
    miniTools.serveText(o.toHtmlDoc({title:'please confirm'}))(req,res);
});

qacServices.organizationServe = function organizationServe(){
    var thisModule = this;
    return app.get(qacServices.rootUrl+':organization',function(req,res,next){
        if(req.params.organization.match(/(.(css|jpg|png|gif|ico|js))$/)) {
            return next();
...
qacServices.adminServe = function adminServe() {
    ...
    return app.get(qacServices.rootUrl+'admin', function(req,res,next){
        ...
        qacServices.getOrganizations().then(function(orgs) {
            var out='<em><form method="post"></em>\n\n';
            out += 'Organizations|Actions\n---|:---:\n';
            for(var o=0; o<orgs.length; ++o) {
                var org = orgs[o];
                out += '['+org+']('+org+') '
                       + '|' +makeButton('Delete', qacServices.rootUrl+'delete/'+org, 'Delete organization \\\''+org+'\\\'')+'\n';
            }
