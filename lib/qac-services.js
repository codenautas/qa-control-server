...
function jsonToFile(filePath, jsonData) {
...
function Bitacora(pathInfo, pathBita) {
    ...
    this.finish = function() {
        ...
        jsonToFile(pathBita+'bitacora.json', fileBita).then(function() {
            var fileAll = logs.filter(isAll);
            return jsonToFile(pathInfo+'bitacora_'+vNow+'.json', fileAll);
        }).then(function() {
...
qacServices.doRepoUpdate = function doRepoUpdate(info, repo_url, feedback, bitacora) {
    ...
    return fs.stat(clonePath).then(function() {
        ...
        return jsonToFile(Path.normalize(resultsPath+'/qa-control-result.json'), warns);
    }).then(function() {
