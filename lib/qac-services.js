...
qacServices.doRepoUpdate = function doRepoUpdate(info, repo_url, feedback, bitacora) {
    ...
    return fs.stat(clonePath).then(function() {
        ...
        return qaControl.loadProject(clonePath);
    }).then(function(info) {
        qaControlInfo = info;
        ...
        if(!label){
            label='ok';
            var clonedQAC = qaControlInfo.packageJson['qa-control'];
            var clonedVersion = clonedQAC ? clonedQAC['package-version'] : '';
            if(clonedVersion === qaControl.currentVersion){
                color='brightgreen';
            }else{
                color='green';
            }
