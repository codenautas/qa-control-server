...
qacServices.receivePush = function receivePush(){
    return app.post('/push/:group/:project',function receivePushService(req,res){
        ...
        qacServices.getInfo(repo.organization, {project:repo.name, createProject:true}).then(function(nfo) {
            ...
            bitacora.logAll('qa-control',warns);
            return fs.writeFile(Path.normalize(resultsPath+'/warnings.json'), JSON.stringify(warns), {encoding:'utf8'});
        }).then(function() {
            ...
            if(haveCucardas) {
                ...
                fs.readFile(cucardasFile, {encoding:'utf8'}).then(function(content) {
