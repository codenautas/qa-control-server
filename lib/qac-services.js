...
function initLoginPlus() {
    if(! qacServices.loginPlusInitialized) {
...
qacServices.createOrganization = function createOrganization(organizationName) {
...
qacServices.overviewServe = function overviewServe(){
    ...
    return app.get('/:organization/:project?',function(req,res,next){
        if(req.params.organization.match(/(.(css|jpg|png|gif))$/)) {
            return res.sendFile(Path.resolve('./app/'+req.params.organization));
        } else if(req.params.organization.match(/^(login|admin|delete)$/)) {
            return next();
        } else {
...
qacServices.deletesServe = function deletesServe() {
    initLoginPlus();
    var thisModule = this;
    return app.get('/delete/:organization/:project?', function(req,res,next){
        thisModule.user = req.session.passport.user;
        thisModule.manageDeletes(req.params.organization, req.params.project).then(function(content) {
           res.end(content); 
        });
    });
};
