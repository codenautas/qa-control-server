...
// qacServices.prueba = function prueba() {
    // return app.get('/prueba', function(req,res,next){
        // qacServices.invalidPage().then(function (content) {
            // res.end(content);
        // });
    // });
// };
...
qacServices.createProject = function createProject(organization, projectName) {
...
function makeLink(organization, project, svg) {
    return '['+project+'](/'+organization+'/'+project+(!!svg?'.svg':'')+')';
}
...
qacServices.makeOverviewMd = function makeOverviewMd(organization, project){
    ...
    return qacServices.getInfo(organization, opts).then(function(info) {
        if(!!project) {
            return fs.readFile(Path.normalize(info.organization.path+'/projects/'+project+'/result/cucardas.md'), 'utf8').then(function(content){
                var r = '  * **'+makeLink(organization,project, true)+'**\n'+content;
                if(qacServices.user) {
                    r += ' [delete](/delete/'+organization+'/'+project+')';
                }
                return r+'\n';
            });
        } else {
            if(info.organization.projects.length) {
                return Promises.all(info.organization.projects.map(function(project) {
                    return fs.readFile(Path.normalize(info.organization.path+'/projects/'+project.projectName+'/result/cucardas.md'), 'utf8').then(function(content){
                        var r = '  * **'+makeLink(organization,project.projectName)+'**\n'+content;
                        if(qacServices.user) {
                            r += ' [delete](/delete/'+organization+'/'+project.projectName+')';
                        }
                        return r+'\n';
                    });
                ...
                return '"'+organization+'" has no projects';
            }
        }
    }).catch(function(err) {
        console.log("makeOverviewMd err", err.stack);
...
qacServices.manageDeletes = function manageDelete(organization, project){
    var opts = project ? { project : project } : null;
    return qacServices.getInfo(organization, opts).then(function(info) {
...
function makeButton(title, link, method, confirmMsg) {
    console.log("makeButton", title, link, method, confirmMsg);
    var sendLink = "this.form.method='"+method+"'; this.form.action='"+link+"'; this.form.submit();";
...
qacServices.adminServe = function adminServe() {
    ...
    return app.get('/admin', function(req,res,next){
        ...
        qacServices.getOrganizations().then(function(orgs) {
            //var out='<em><form method="'+method+'"></em>\n\n';
            var out='<em><form></em>\n\n';
            out += 'Organizations|Actions\n--- | --- | ---\n';
            for(var o=0; o<orgs.length; ++o) {
                var org = orgs[o];
                out += org + '|' + makeButton('Browse', '/'+org, 'get')+' '+ makeButton('Delete', '/'+accPref+'delete/'+org, method, 'Desea eliminar la organizacion \\\''+org+'\\\'')+'\n';
                //out += '  * ['+org+'](/'+org+')  [delete](/delete/'+org+')\n';
            }
            out += '\n<em></form></em>';
...
function handleAbms(thisModule) {
    this.handle = function(req, res, next) {
        ...
        switch(req.params.action) {
            case actionPref+'delete': doAction = thisModule.manageDeletes; break;
            case actionPref+'create': doAction = thisModule.createOrganization; break; // ignora req.params.project
        }
